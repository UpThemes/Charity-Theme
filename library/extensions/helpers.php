...
function charity_trim_excerpt($text) {
	if ( '' == $text ) {
...
function charity_the_excerpt($deprecated = '') {
	global $post;
	$output = '';
...
function charity_excerpt_rss() {
	...
	return apply_filters('charity_excerpt_rss', $output);

}

add_filter('charity_excerpt_rss', 'charity_trim_excerpt');

// create nice multi_tag_title
// Credits: Martin Kopischke for providing this code

function charity_tag_query() {
...
function charity_get_term_name() {
	// Credits: Justin Tadlock Theme Hybrid
	$term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) ); 
	return $term->name;
}

function charity_is_custom_post_type() {
	global $post; 
