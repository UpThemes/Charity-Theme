...
function charity_bodyopen() {
    if (apply_filters('charity_show_bodyclass',TRUE)) { 
        // Creating the body class
    	if (!(CHARITY_COMPATIBLE_BODY_CLASS)) { 
    		echo '<body ';
    		body_class();
    		echo '>' . "\n\n";
    	} else { 
...
if (function_exists('childtheme_override_body_class'))  {
	function charity_body_class() {
		childtheme_override_body_class();
	}
} else {
	// Generates semantic classes for BODY element
	function charity_body_class( $print = true ) {
		global $wp_query, $current_user, $blog_id, $post, $taxonomy;
	    
	    $c = array();
	
		if (apply_filters('charity_show_bc_wordpress', TRUE)) {
	        // It's surely a WordPress blog, right?
	        $c[] = 'wordpress';
	    }
	    
	    if (apply_filters('charity_show_bc_blogid', TRUE)) {
	    	// Applies the blog id to BODY element .. blog-id1 for WordPress < 3.0
	    	$c[] = 'blogid-' . $blog_id;
	    }
	
		if (apply_filters('charity_show_bc_datetime', TRUE)) {
	        // Applies the time- and date-based classes (below) to BODY element
	        charity_date_classes( time(), $c );
	    }
	
	    if (apply_filters('charity_show_bc_contenttype', TRUE)) {
	    ...
	    if (apply_filters('charity_show_bc_singular', TRUE)) {
		...
		if ( is_single() && apply_filters('charity_show_bc_singlepost', TRUE)) {
			...
			if ( isset( $wp_query->post->post_date ) )
				charity_date_classes( mysql2date( 'U', $wp_query->post->post_date ), $c, 's-' );
		...
		elseif ( is_author() && apply_filters('charity_show_bc_authorarchives', TRUE)) {
		...
		elseif ( is_category() && apply_filters('charity_show_bc_categoryarchives', TRUE)) {
		...
		elseif ( is_tag() && apply_filters('charity_show_bc_tagarchives', TRUE)) {
		...
		elseif ( is_tax() && apply_filters('charity_show_bc_taxonomyarchives', TRUE)) {
			$c[] = 'taxonomy';
			$c[] = 'tax-' . $taxonomy;
			$c[] = $taxonomy . '-' . strtolower(charity_get_term_name());
		}
	
		// Page author for BODY on 'pages'
		elseif ( is_page() && apply_filters('charity_show_bc_pages', TRUE)) {
		...
		elseif ( is_search() && apply_filters('charity_show_bc_search', TRUE)) {
			the_post();
		...
		if (apply_filters('charity_show_bc_loggedin', TRUE)) {
	 ...
	 // Paged classes; for page x > 1 classes of index and all post types etc.
		if (apply_filters('charity_show_bc_pagex', TRUE)) {
			if ( (( ( $page = $wp_query->get('paged') ) || ( $page = $wp_query->get('page') ) ) && $page > 1 ) ) {
				...
				$page = intval($page); // Ensures that an integer (not some dangerous script) is passed for the variable
 					$c[] = 'paged-' . $page;
 				if (charity_is_custom_post_type()) {
		...
		$c = join( ' ', apply_filters( 'charity_body_class',  $c ) ); // Available filter: charity_body_class
...
if (apply_filters('charity_show_bc_browser', TRUE) ) {
	add_filter('charity_body_class', 'browser_class_names', 20); 
	}
...
if (function_exists('childtheme_override_post_class'))  {
	function charity_post_class() {
		childtheme_override_post_class();
	}
} else {
	// Generates semantic classes for each post DIV element
	function charity_post_class( $print = true ) {
		global $post, $charity_post_alt, $charity_content_length, $taxonomy;
	
		// hentry for hAtom compliace, gets 'alt' for every other post DIV, describes the post type and p[n]
		$c = array( 'hentry', "p$charity_post_alt", str_replace('_','-',$post->post_type), $post->post_status );
		...
		if ($charity_content_length == 'full')
			$c[] = 'is-full';

		// For posts displayed as excerpts
		if ($charity_content_length == 'excerpt') {
			$c[] = 'is-excerpt';
		...
		charity_date_classes( mysql2date( 'U', $post->post_date ), $c );
	
		// If it's the other to the every, then add 'alt' class
		if ( ++$charity_post_alt % 2 )
			$c[] = 'alt';
...
if (function_exists('childtheme_override_comment_class'))  {
	function charity_comment_class() {
		childtheme_override_comment_class();
	}
} else {
	// Generates semantic classes for each comment LI element
	function charity_comment_class( $print = true ) {
		global $comment, $post, $charity_comment_alt, $comment_depth, $comment_thread_alt;
		...
		if ( $comment->comment_type == 'comment' ) {
			$c[] = "c$charity_comment_alt";
		} else {
			$c[] = "t$charity_comment_alt";
		}
		...
		charity_date_classes( mysql2date( 'U', $comment->comment_date ), $c, 'c-' );
		if ( ++$charity_comment_alt % 2 )
			$c[] = 'alt';
...
if (function_exists('childtheme_override_date_classes'))  {
	function charity_date_classes() {
		childtheme_override_date_classes();
	}
} else {
	function charity_date_classes( $t, &$c, $p = '' ) {
...
// Remember: Charity, like The Sandbox, is for play.
?>